...
class BootProgressPublisher : public PostObject
{
  ...
  private:
    ...
    sdbusplus::async::task<void>
        updateBootProgressProperty(const std::string& progressStage);
    sdbusplus::async::task<void>
        updateBootProgressLastUpdateProperty(uint32_t timeStamp);
    sdbusplus::async::task<void>
        updateBootProgressOemProperty(const std::string& oemLastState);
